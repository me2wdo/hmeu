...
async def get_backup(c: Client, m: Message):
    await c.send_document(m.chat.id, "alqaid.db", reply_to_message_id=m.id)


async def get_backup2(c: Client, m: Message):
    await c.send_document(m.chat.id, "alqaid2.db", reply_to_message_id=m.id)


async def upper_backup(c: Client, m: Message):
    if re.match("^theo\.db$", str(m.reply_to_message.document.file_name)):
        await c.download_media(m.reply_to_message, file_name="./alqaid.db")
        os.chmod('alqaid.db', 0o0777)
